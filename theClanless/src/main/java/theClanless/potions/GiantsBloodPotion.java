...
public class GiantsBloodPotion extends CustomPotion {

    public static final String POTION_ID = theClanlessMod.makeID("GiantsBloodPotion");
    private static final PotionStrings potionStrings = CardCrawlGame.languagePack.getPotionString(POTION_ID);

    public static final String NAME = potionStrings.NAME;
    public static final String[] DESCRIPTIONS = potionStrings.DESCRIPTIONS;

    public GiantsBloodPotion() {
        // The bottle shape and inside is determined by potion size and color. The actual colors are the main DefaultMod.java
        super(NAME, POTION_ID, PotionRarity.UNCOMMON, PotionSize.S, PotionColor.ELIXIR);
    ...
    public void use(AbstractCreature target) {
        ...
        if (AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.COMBAT) {
            addToBot(new AddTemporaryHPAction(target, target, potency*(Math.max(0, target.maxHealth - (target.currentHealth + TempHPField.tempHp.get(target))))));
        }
    }
